...
use kayrx::codec::Framed2 as Framed;
use kayrx::krse::net::TcpStream;
use kayrx::kcore::{Server, ServiceFactory};
use kayrx::web::client::{self, error::PayloadError, ws, Client, ClientRequest, ClientResponse, Connector};
use bytes::Bytes;
use futures::Stream;
use http::Method;
use net2::TcpBuilder;
...
///
/// ```rust
/// use kayrx::http::HttpService;
/// use kayrx_test::http::TestServer;
/// use kayrx::web::{web, App, HttpResponse, Error};
///
...
pub fn test_server<F: ServiceFactory<TcpStream>>(factory: F) -> TestServer {
    ...
    kayrx::connect::start_default_resolver();
...
impl TestServer {
    ...
    pub async fn ws_at(
        &mut self,
        path: &str,
    ) -> Result<Framed<impl AsyncRead + AsyncWrite, ws::Codec>, client::error::WsClientError>
    {
