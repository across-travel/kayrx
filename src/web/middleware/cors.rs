...
impl Cors {
    ...
    /// CredentialsWithWildcardOrigin` error during kayrx launch or runtime.
    ///
    /// Defaults to `false`.
...
impl Inner {
    pub fn validate_origin(&self, req: &RequestHead) -> Result<(), CorsError> {
    ...
    pub fn validate_allowed_method(&self, req: &RequestHead) -> Result<(), CorsError> {
    ...
    pub fn validate_allowed_headers(&self, req: &RequestHead) -> Result<(), CorsError> {
        match self.headers {
            AllOrSome::All => Ok(()),
...
// #[cfg(test)]
// mod tests {
//     use crate::service::{fn_service, Transform};
