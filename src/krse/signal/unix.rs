...
use crate::krse::io::{AsyncRead, PollEvented};
...
impl SignalKind {
    ...
    /// signals.
    /// ```rust,no_run
    /// # use kayrx::krse::signal::unix::SignalKind;
    /// # let signum = -1;
...
fn signal_enable(signal: c_int) -> io::Result<()> {
    if signal < 0 || hook::FORBIDDEN.contains(&signal) {
    ...
    siginfo.init.call_once(|| {
        registered = unsafe {
            hook::register(signal, move || action(globals, signal)).map(|_| ())
        };
        if registered.is_ok() {
...
impl Signal {
    ...
    /// use std::task::{Context, Poll};
    /// use kayrx::krse::signal::unix::Signal;
    ///
    /// struct MyFuture {
    ///     signal: Signal,
    /// }
    ...
    impl futures_core::stream::Stream for Signal {
        type Item = ();
...
mod tests {
    ...
    fn signal_enable_error_on_forbidden_input() {
        signal_enable(hook::FORBIDDEN[0]).unwrap_err();
    }
}
