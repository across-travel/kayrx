...
#[derive(Debug)]
pub struct MayAcquireError(());

impl Semaphore {
    /// Creates a new semaphore with the initial number of permits
    pub fn new(permits: usize) -> Self {
        Self {
            inner: SemaphoreInner::new(permits),
        }
    }
    ...
    pub async fn acquire(&self) -> SemaphorePermit<'_> {
        ...
        poll_fn(|cx| permit.inner.poll_acquire(cx, 1, &self.inner))
            .await
            .unwrap();
        permit
    }
    ...
    pub fn try_acquire(&self) -> Result<SemaphorePermit<'_>, MayAcquireError> {
        let mut inner = Permit::new();
        match inner.try_acquire(1, &self.inner) {
            Ok(_) => Ok(SemaphorePermit {
                sem: self,
                inner,
            }),
            Err(_) => Err(MayAcquireError(())),
        }
    }
}
...
impl<'a> Drop for SemaphorePermit<'_> {
    fn drop(&mut self) {
        self.inner.release(1, &self.sem.inner);
    }
}
...
impl SemaphoreInner {
    ...
    pub(crate) fn new(permits: usize) -> SemaphoreInner {
...
impl fmt::Debug for SemaphoreInner {
...
impl Permit {
    ...
    pub(crate) fn release(&mut self, n: u16, semaphore: &SemaphoreInner) {
        let n = self.forget(n);
