        ...
        async move {
            ...
            for token in tokens {
                if let Some(srv) = services.remove(&token) {
                    ...
                    res.push((
                        token,
                        Box::new(StreamService::new(kayrx::fn_service(
                            move |_: TcpStream| {
...
pub(crate) trait ServiceRuntimeConfiguration: Send {
...
impl ServiceRuntime {
    ...
    where
        F: kayrx::IntoServiceFactory<T>,
        T: kayrx::ServiceFactory<Config = (), Request = TcpStream> + 'static,
        T::Future: 'static,
        T::Service: 'static,
...
impl<T> kayrx::ServiceFactory for ServiceFactory<T>
where
    T: kayrx::ServiceFactory<Config = (), Request = TcpStream>,
    T::Future: 'static,
    T::Service: 'static,
    T::Error: 'static,
