//! The node module exposes the `VNode` struct and methods that power our
//! virtual dom.
...
use crate::webui::vdom::validation;
...
/// `VNode`'s.
///
/// `html! { <div> <span></span> </div> }` really generates a `VNode` with
/// one child (span).
///
...
/// TODO: Create a builder to create instances of VNode::Element with
...
impl VElement {
    ...
    pub fn create_element_node(&self) -> CreatedNode<Element> {
        ...
        let element = if validation::is_svg_namespace(&self.tag) {
            document
...
impl VText {
    ...
    /// Return a `Text` element from a `VNode`, typically right before adding it
    /// into the DOM.
...
pub trait View {
    /// Render a VNode, or any IntoIter<VNode>
    fn render(&self) -> VNode;
}

impl<V> From<&V> for VNode
where
    V: View,
{
    fn from(v: &V) -> Self {
...
impl fmt::Display for VElement {
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !validation::is_self_closing(&self.tag) {
            write!(f, "</{}>", self.tag)?;
        }
...
impl fmt::Debug for Events {
    // Print out all of the event names for this VNode
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
