...
use crate::http::h2::codec::{Codec, RecvError, SendError, UserError};
use crate::http::h2::frame::{Headers, Pseudo, Reason, Settings, StreamId};
use crate::http::h2::proto;
use crate::http::h2::{FlowControl, PingPong, RecvStream, SendStream};
...
use std::usize;
use crate::krse::io::{AsyncRead, AsyncWrite, AsyncWriteExt};
    ...
    B: Buf + 'static,
{
    ...
    ///
    /// [module]: index.html
    pub fn poll_ready(&mut self, cx: &mut Context) -> Poll<Result<(), crate::http::h2::Error>> {
    ...
    pub fn send_request(
        ...
        request: Request<()>,
        end_of_stream: bool,
    ) -> Result<(ResponseFuture, SendStream<B>), crate::http::h2::Error> {
        self.inner
...
where
    B: Buf + 'static,
{
    type Output = Result<SendRequest<B>, crate::http::h2::Error>;
...
impl Builder {
    ...
    pub fn handshake<T, B>(
        &self,
        io: T,
    ) -> impl Future<Output = Result<(SendRequest<B>, Connection<T, B>), crate::http::h2::Error>>
    where
...
/// # pub fn main() {}
/// ```
pub async fn handshake<T>(io: T) -> Result<(SendRequest<Bytes>, Connection<T, Bytes>), crate::http::h2::Error>
where
    T: AsyncRead + AsyncWrite + Unpin,
{
    ...
    B: Buf + 'static,
{
    async fn handshake2(
        mut io: T,
        builder: Builder,
    ) -> Result<(SendRequest<B>, Connection<T, B>), crate::http::h2::Error> {
        log::debug!("binding client connection");

        let msg: &'static [u8] = b"PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n";
        io.write_all(msg).await.map_err(crate::http::h2::Error::from_io)?;
    ...
    pub fn set_initial_window_size(&mut self, size: u32) -> Result<(), crate::http::h2::Error> {
        assert!(size <= proto::MAX_WINDOW_SIZE);
...
where
    ...
    B: Buf + 'static,
{
    type Output = Result<(), crate::http::h2::Error>;
...
impl Future for ResponseFuture {
    type Output = Result<Response<RecvStream>, crate::http::h2::Error>;
...
impl ResponseFuture {
    ...
    pub fn stream_id(&self) -> crate::http::h2::StreamId {
        crate::http::h2::StreamId::from_internal(self.inner.stream_id())
    }
...
impl PushPromises {
    ...
    pub async fn push_promise(&mut self) -> Option<Result<PushPromise, crate::http::h2::Error>> {
    ...
    pub fn poll_push_promise(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Option<Result<PushPromise, crate::http::h2::Error>>> {
...
impl futures_core::Stream for PushPromises {
    type Item = Result<PushPromise, crate::http::h2::Error>;
