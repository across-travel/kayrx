...
use crate::http::h2::frame::{self, Frame, Reason};
use crate::http::h2::proto::{peer, Open, Peer, WindowSize};
use crate::http::h2::{client, proto, server};
    ...
    B: Buf,
{
    pub fn poll_pending_open(
        ...
        cx: &Context,
        pending: Option<&OpaqueStreamRef>,
    ) -> Poll<Result<(), crate::http::h2::Error>> {
        let mut me = self.inner.lock().unwrap();
        let me = &mut *me;
...
impl<B> StreamRef<B> {
    ...
    pub fn send_push_promise(&mut self, request: Request<()>) -> Result<StreamRef<B>, UserError> {
        ...
        let pushed = {
            ...
            let frame = crate::http::h2::server::Peer::convert_push_message(stream.id, promised_id, request)?;
    ...
    ) -> Poll<Result<Reason, crate::http::h2::Error>> {
        let mut me = self.opaque.inner.lock().unwrap();
        let me = &mut *me;
