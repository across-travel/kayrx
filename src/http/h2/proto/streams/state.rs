...
use crate::http::h2::codec::{RecvError, UserError};
use crate::http::h2::frame::Reason;
use crate::http::h2::proto::{self, PollReset};
...
impl State {
    ...
    pub(super) fn ensure_reason(&self, mode: PollReset) -> Result<Option<Reason>, crate::http::h2::Error> {
        match self.inner {
            Closed(Cause::Proto(reason))
            | Closed(Cause::LocallyReset(reason))
