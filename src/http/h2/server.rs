...
use crate::http::h2::proto::{self, Config, Prioritized};
    ...
    B: Buf + 'static,
{
    ...
    pub async fn accept(
        &mut self,
    ) -> Option<Result<(Request<RecvStream>, SendResponse<B>), crate::http::h2::Error>> {
    ...
    pub fn poll_accept(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Option<Result<(Request<RecvStream>, SendResponse<B>), crate::http::h2::Error>>> {
    ...
    /// [`RecvStream`]: ../struct.RecvStream.html
    /// [`SendStream`]: ../struct.SendStream.html
    pub fn poll_closed(&mut self, cx: &mut Context) -> Poll<Result<(), crate::http::h2::Error>> {
        self.connection.poll(cx).map_err(Into::into)
    }

    #[doc(hidden)]
    #[deprecated(note = "renamed to poll_closed")]
    pub fn poll_close(&mut self, cx: &mut Context) -> Poll<Result<(), crate::http::h2::Error>> {
        self.poll_closed(cx)
    }
...
where
    ...
    B: Buf + 'static,
{
    type Item = Result<(Request<RecvStream>, SendResponse<B>), crate::http::h2::Error>;
...
impl<B: Buf> SendResponse<B> {
    ...
    pub fn send_response(
        ...
        response: Response<()>,
        end_of_stream: bool,
    ) -> Result<SendStream<B>, crate::http::h2::Error> {
        self.inner
    ...
    pub fn push_request(
        &mut self,
        request: Request<()>,
    ) -> Result<SendPushedResponse<B>, crate::http::h2::Error> {
        self.inner
    ...
    /// a user error.
    pub fn poll_reset(&mut self, cx: &mut Context) -> Poll<Result<Reason, crate::http::h2::Error>> {
...
where
    ...
    B: Buf,
{
    type Output = Result<Codec<T, B>, crate::http::h2::Error>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        // Flush the codec
        ready!(self.codec.as_mut().unwrap().flush(cx)).map_err(crate::http::h2::Error::from_io)?;
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        while rem > 0 {
            let n = ready!(Pin::new(self.inner_mut()).poll_read(cx, &mut buf[..rem]))
                .map_err(crate::http::h2::Error::from_io)?;
            if n == 0 {
                return Poll::Ready(Err(crate::http::h2::Error::from_io(io::Error::new(
...
where
    ...
    B: Buf + 'static,
{
    type Output = Result<Connection<T, B>, crate::http::h2::Error>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        log::trace!("Handshake::poll(); state={:?};", self.state);
        use crate::http::h2::server::Handshaking::*;
