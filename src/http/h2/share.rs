use crate::http::h2::codec::UserError;
use crate::http::h2::frame::Reason;
use crate::http::h2::proto::{self, WindowSize};

use bytes::{Buf, Bytes};
use http::HeaderMap;

use crate::http::h2::PollExt;
use std::fmt;
use std::pin::Pin;
...
impl<B: Buf> SendStream<B> {
    ...
    /// sent data.
    pub fn poll_capacity(&mut self, cx: &mut Context) -> Poll<Option<Result<usize, crate::http::h2::Error>>> {
        self.inner
            .poll_capacity(cx)
    ...
    pub fn send_data(&mut self, data: B, end_of_stream: bool) -> Result<(), crate::http::h2::Error> {
        self.inner
    ...
    pub fn send_trailers(&mut self, trailers: HeaderMap) -> Result<(), crate::http::h2::Error> {
...
impl StreamId {
    pub(crate) fn from_internal(id: crate::http::h2::frame::StreamId) -> Self {
        StreamId(id.into())
    }
}
// ===== impl RecvStream =====
...
impl RecvStream {
    ...
    pub async fn data(&mut self) -> Option<Result<Bytes, crate::http::h2::Error>> {
        futures_util::future::poll_fn(move |cx| self.poll_data(cx)).await
    }

    /// Get optional trailers for this stream.
    pub async fn trailers(&mut self) -> Result<Option<HeaderMap>, crate::http::h2::Error> {
        futures_util::future::poll_fn(move |cx| self.poll_trailers(cx)).await
    }

    #[doc(hidden)]
    pub fn poll_data(&mut self, cx: &mut Context<'_>) -> Poll<Option<Result<Bytes, crate::http::h2::Error>>> {
    ...
    pub fn poll_trailers(
        &mut self,
        cx: &mut Context,
    ) -> Poll<Result<Option<HeaderMap>, crate::http::h2::Error>> {
...
impl futures_core::Stream for RecvStream {
    type Item = Result<Bytes, crate::http::h2::Error>;
...
impl FlowControl {
    ...
    ///
    /// [struct level]: #
    pub fn release_capacity(&mut self, sz: usize) -> Result<(), crate::http::h2::Error> {
...
impl PingPong {
    ...
    pub async fn ping(&mut self, ping: Ping) -> Result<Pong, crate::http::h2::Error> {
        self.send_ping(ping)?;
        futures_util::future::poll_fn(|cx| self.poll_pong(cx)).await
    }

    #[doc(hidden)]
    pub fn send_ping(&mut self, ping: Ping) -> Result<(), crate::http::h2::Error> {
    ...
    #[doc(hidden)]
    pub fn poll_pong(&mut self, cx: &mut Context) -> Poll<Result<Pong, crate::http::h2::Error>> {
