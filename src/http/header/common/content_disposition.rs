...
use crate::http::header::{self, ExtendedValue, Header, IntoHeaderValue, Writer};
...
/// # Examples
/// ```
/// use crate::http::header::DispositionParam;
///
...
impl ContentDisposition {
    ...
    pub fn from_raw(hv: &header::HeaderValue) -> Result<Self, crate::http::error::ParseError> {
        ...
        let hv = String::from_utf8(hv.as_bytes().to_vec())
            .map_err(|_| crate::http::error::ParseError::Header)?;
        let (disp_type, mut left) = split_once_and_trim(hv.as_str().trim(), ';');
        if disp_type.is_empty() {
            return Err(crate::http::error::ParseError::Header);
        }
        ...
        while !left.is_empty() {
            ...
            if param_name.ends_with('*') {
                ...
                let value = if left.starts_with('\"') {
                    ...
                    left = &left[end.ok_or(crate::http::error::ParseError::Header)? + 1..];
...
impl Header for ContentDisposition {
    ...
    fn parse<T: crate::http::HttpMessage>(msg: &T) -> Result<Self, crate::http::error::ParseError> {
        if let Some(h) = msg.headers().get(&Self::name()) {
            Self::from_raw(&h)
        } else {
            Err(crate::http::error::ParseError::Header)
        }
    }
}
...
mod tests {
    ...
    use crate::http::header::shared::Charset;
