...
/// `kayrx::http::error::Error` directly and is otherwise a direct mapping to
/// `Result`.
pub type Result<T, E = Error> = result::Result<T, E>;

/// General purpose kayrx::web error.
///
/// An kayrx::web error is used to carry errors from `std::error`
/// through kayrx in a convenient way.  It can be created through
/// converting errors with `into()`.
///
/// Whenever it is created from an external object a response error is created
/// for it that can be used to create an http response from it this means that
/// if you have access to an kayrx `Error` you can always get a
...
pub enum PayloadError {
    ...
    /// Http2 payload error
    #[display(fmt = "{}", _0)]
    Http2Payload(crate::http::h2::Error),
    /// Io error
    #[display(fmt = "{}", _0)]
    Io(io::Error),
}

impl From<crate::http::h2::Error> for PayloadError {
    fn from(err: crate::http::h2::Error) -> Self {
