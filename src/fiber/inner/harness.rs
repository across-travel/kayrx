use crate::fiber::inner::fiber::{Cell, Core, Header, Trailer};
use crate::fiber::inner::state::Snapshot;
use crate::fiber::inner::{JoinError, Schedule, Fiber};
use crate::krse::cell::CausalCheck;
    ...
    S: Schedule,
{
    ...
    pub(super) unsafe fn poll(mut self, executor: &mut dyn FnMut() -> Option<NonNull<()>>) -> bool {
        ...
        if header.executor().is_none() {
            ...
            // own the task here.
            let task = ManuallyDrop::new(Fiber::from_raw(header.into()));
    ...
    unsafe fn to_task(&self) -> Fiber<S> {
        let ptr = self.cell.as_ptr() as *mut Header;
        Fiber::from_raw(NonNull::new_unchecked(ptr))
    }
}
