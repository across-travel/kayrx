use crate::fiber::inner::{Cell, Harness, Snapshot, State, Header, Schedule, ScheduleSend};
...
impl RawFiber {
    ...
    fn new<T, S>(task: T, state: State) -> RawFiber
    where
        T: Future + 'static,
        S: Schedule,
    {
    ...
    pub(super) fn new_joinable<T, S>(task: T) -> RawFiber
    where
        T: Future + Send + 'static,
        S: ScheduleSend,
    {
        RawFiber::new::<_, S>(task, State::new_joinable())
    }

    pub(super) fn new_joinable_local<T, S>(task: T) -> RawFiber
    where
        T: Future + 'static,
        S: Schedule,
    {
    ...
    pub(super) unsafe fn from_raw(ptr: NonNull<Header>) -> RawFiber {
        RawFiber { ptr }
    }
...
impl Clone for RawFiber {
    fn clone(&self) -> Self {
        RawFiber { ptr: self.ptr }
    }
}

impl Copy for RawFiber {}
