use crate::fiber::inner::{Header, Fiber};
...
impl<T: 'static> TransferStack<T> {
    ...
    pub(crate) fn push(&self, task: Fiber<T>) {
        unsafe {
    ...
    pub(crate) fn drain(&self) -> impl Iterator<Item = Fiber<T>> {
        struct Iter<T: 'static>(*mut Header, PhantomData<T>);

        impl<T: 'static> Iterator for Iter<T> {
            type Item = Fiber<T>;

            fn next(&mut self) -> Option<Fiber<T>> {
