use crate::fiber::inner::{self as fiber, queue::MpscQueues, Schedule, ScheduleSend, Fiber};
use crate::fiber::{JoinHandle, context};
    ...
    P: Park,
{
    ...
    {
        let (task, handle) = fiber::joinable(future);
        ...
        F: Future,
    {
        ...
        context::with_basic_scheduler(scheduler, || {
            let mut _enter = crate::fiber::enter::enter();
...
impl Spawner {
    ...
    where
        F: FnOnce() -> R,
    {
        context::with_basic_scheduler(&*self.scheduler, f)
    }
}
...
impl SchedulerPriv {
    ...
    fn schedule(&self, task: Fiber<Self>, spawn: bool) {
...
impl ScheduleSend for SchedulerPriv {}
...
impl Schedule for NoopSchedule {
    fn bind(&self, _task: &Fiber<Self>) {}

    fn release(&self, _task: Fiber<Self>) {}

    fn release_local(&self, _task: &Fiber<Self>) {}

    fn schedule(&self, _task: Fiber<Self>) {
        unreachable!();
    }
}
